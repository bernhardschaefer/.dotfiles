...
# Extract model, directory, cwd, cost, and duration
...
# Build first line: Model | Dir | Branch (uncommitted)
line1="${C_ACCENT}${model}${C_GRAY} | 📁${dir}"
[[ -n "$branch" ]] && line1+=" | 🔀${branch} ${git_status}"
line1+="${C_RESET}"

# Build second line: Context | Cost | Duration
line2="${ctx} ${C_GRAY}| ${C_ACCENT}${cost_fmt}${C_GRAY} | ⏱️ ${mins}m ${secs}s${C_RESET}"
...
if [[ -n "$transcript_path" && -f "$transcript_path" ]]; then
    # Calculate visible length based on second line (longest line)
    plain_output="xxxxxxxxxx ${pct}% of ${max_k}k tokens | ${cost_fmt} | ⏱️ ${mins}m ${secs}s"
